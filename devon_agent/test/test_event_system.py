...
def test_revert_to_last_checkpoint(event_system, mock_environment, mock_agent):
    ...
    event_system.add_event(Event("user1", "session1", "traj1", "versioning", "checkpoint", "request", "producer1", "content1"))
    event_system.get_event()  # Process the event and create a checkpoint
    
    event_system.add_event(Event("user1", "session1", "traj1", "custom", "test", "action", "producer2", "content2"))
...
def test_revert_to_last_checkpoint_running_error(event_system):
    ...
    with pytest.raises(Exception, match="Event system is running, cannot revert to last checkpoint"):
        event_system.revert_to_last_checkpoint()

def test_reset(event_system):
    event_system.add_event(Event("user1", "session1", "traj1", "custom", "test", "action", "producer1", "content1"))
    event_system.get_event()
