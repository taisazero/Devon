...
class Event:
    ...
    __slots__ = ["user", "session_name", "trajectory_id", "event_type", "sub_event", "action", "producer", "content", "metadata", "consumer"]

    def __init__(self, user : str, session_name : str, trajectory_id : str, event_type : str, sub_event : str, action : str,producer : str, content : Any, metadata : Optional[Dict[str, Any]]=None, consumer : Optional[str]=None):
        self.user = user
    ...
    def __hash__(self):
        return hash((self.user, self.session_name, self.trajectory_id, self.event_type, self.sub_event, self.action, self.producer, self.content, self.metadata, self.consumer))
...
class EventSystem:
    ...
    def __init__(self):
        ...
        self.event_handlers : Dict[Tuple[str, str, str], Callable[[EventSystem, Event], List[Event]]] = {}

    def register_environment(self, name, environment : EnvironmentModule):
        assert self.status == "paused", "Event system is running, cannot register environment"
    ...
    def revert_to_last_checkpoint(self):
        assert self.status == "paused", "Event system is running, cannot revert to last checkpoint"
    ...
    def process_event(self, event : Event):
        if (event.event_type, event.sub_event,event.action) in self.event_handlers:
            events = self.event_handlers[(event.event_type, event.sub_event, event.action)](self,event)
